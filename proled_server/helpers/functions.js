...
const functionsBD = require("./functionsBD");

function createGame(games, player, roomId, googleUserData) {
	console.log("Nuevo");
	games.push({
		id: Date.now(),
		roomId: roomId,
...
exports.findGame = (games, player, roomId, googleUserData) => {
	//Añadimos jugador a partida
	if (games.length === 0) {
		createGame(games, player, roomId, googleUserData);
	} else if (games[games.length - 1].player2 === null) {
		console.log("actualizar");
		games[games.length - 1].player2 = player;
		games[games.length - 1].dataPlayer2 = googleUserData;
		console.log(games);
		return 1;
	} else {
		createGame(games, player, roomId, googleUserData);
...
exports.getWinner = (cellArrayGame) => {
  ...
  */
	...
	for (var i = 0; i < aGame.length; i++) {
		j = i;
		if (i >= 0 && i <= 2) gameState[0][i] = aGame[i] == 2 ? -1 : aGame[i] == 1 ? 1 : 0;
		if (i >= 3 && i <= 5) gameState[1][j - 3] = aGame[i] == 2 ? -1 : aGame[i] == 1 ? 1 : 0;
		if (i >= 6 && i <= 8) gameState[2][j - 6] = aGame[i] == 2 ? -1 : aGame[i] == 1 ? 1 : 0;
	}
	/*
...
exports.findPlayerGame = (games, socketId) => {
	...
	var results = games.filter((o) => o.player1 === socketId || o.player2 === socketId);
...
exports.addCellPushedToArrayGame = (id, oGame, socketId) => {
	...
	if (oGame.cellArray.length == 0) oCell = { id: id, image: 1, socketId: socketId };
	else {
		oCell = {
			id: id,
...
//Chequear que un jugador no pulse en su turno más de una celda
exports.checkSeveralCellsPushed = (oGame, socketId) => {
	if (oGame == null || oGame.cellArray.length == 0) return false;
	return oGame.cellArray[oGame.cellArray.length - 1].socketId === socketId ? true : false;
};

//registrar información de la partida
exports.storeGame = (oGame, winner) => {
	// Comprobar  existencia de Jugadores en Base de datos y en caso de no estar darlos de alta

	//Jugador1
	functionsBD.insertWithCheckPlayerInDb(oGame.dataPlayer1.email, oGame.dataPlayer1.photo, oGame.dataPlayer1.name);

	//Jugador2
	functionsBD.insertWithCheckPlayerInDb(oGame.dataPlayer2.email, oGame.dataPlayer2.photo, oGame.dataPlayer2.name);

	// Grabar Partida
	functionsBD.insertGame(oGame, winner);
};
