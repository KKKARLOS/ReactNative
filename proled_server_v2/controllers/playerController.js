...
exports.getAllPlayers = async (req, res) => {
	...
	await Player.find({}, (err, players) => {
		Country.populate(players, { path: "_id_country" }, function (err, players) {
...
//db.players.findOne({_id:ObjectId("5eb1d21d9d5a435684950952")}).populate('countries');
exports.getPlayerById = async (req, res) => {
	let playerId = req.params.id;

	await Player.findOne({ _id: playerId }, (err, player) => {
		Country.populate(player, { path: "_id_country" }, function (err, player) {
			if (err) return res.status(500).send({ message: `Error al realizar la petición: ${err}` });
			if (!player) return res.status(404).send({ message: `No existen jugadores` });
...
exports.insertPlayer = async (req, res) => {
	...
	player._id_country = req.body._id_country;
